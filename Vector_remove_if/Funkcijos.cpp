...
void skirstymas(vector <data> & var, vector <data> & temp, int moksk){
    Timer hvar;
    
    var = temp; temp.erase(temp.begin()); var.erase(var.begin());
    temp.erase(std ::remove_if(temp.begin(), temp.end(),daugiau), temp.end());
    var.erase(std ::remove_if(var.begin(), var.end(),[]( const data& a ){return a.vid >= 5.0; }), var.end());

    cout << moksk << " Galvociu ir vargsiuku studentu rusiavimas uztruko: " << hvar.elapsed() << " s\n";
}
bool daugiau( const data& a ){return a.vid < 5.0; }
