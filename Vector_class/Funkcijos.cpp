...
void ivestis(Vector<studentas>& temp){
    ...
    while(n!="B"){
        ...
        if (n == "B") break;
        else if (n == "A" && ArIvede == false){
            cout << "Kiek pazymiu norite sugeneruoti?" << endl;
            string sk ("-1");
            while (stoi(sk)<0 || !Ar_tai_yra_skaicius(sk)){
                cin >> sk;
                if (!Ar_tai_yra_skaicius(sk) || stoi(sk)<0)
            ...
            int x = stoi(n);
            if (x > 10 || x<1) cout << "ivestas skaicius neatitinka kriterijaus"<<endl;
            else {
                temp.back().NewPaz(x);
...
void nuskaitymas(Vector <studentas> &temp, ifstream& df){
  ...
  while (zodis != "Egz."){
    ...
    if (!df.eof( ))  temp.push_back(studentas());
    }
