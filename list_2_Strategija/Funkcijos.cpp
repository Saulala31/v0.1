...
void ivestis(list<data>& temp){
    ...
    if (temp.back().paz_sk != 0){
        int suma = 0; 
        list <int>::iterator it = temp.back().paz.begin();
        for (int i=0; i<temp.back().paz_sk; i++)
        advance(it, i);
        suma = suma + *it;
    ...
    while(std::getline(lineStream, cell, ' ' ))
                {
                    if(cell != ""){
                            ...
                            if (temp.back().paz_suma != 0){
                                temp.back().vid = (temp.back().paz_suma/nd_sk)*0.4+0.6*temp.back().egz;
                                temp.back().med = Mediana(temp.back().paz);
                            }
                            else temp.back().vid = 0.6*temp.back().egz;
                    }
...
void skirstymas(list <data> & var, list <data> & temp, int moksk){
    Timer hvar;
    list<data>::iterator itri = temp.begin(); temp.erase(itri++);
 while (  itri != temp.end() )
 {
    if (itri->vid < 5.0)
    {
